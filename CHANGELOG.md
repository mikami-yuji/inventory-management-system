...
- **特売編集画面での商品更新・追加時に特売引当数が反映されないバグの修正**:
  - 特売イベントの編集において、新規で商品が追加された際、特売引当数 (`allocated_quantity`) が `0` のまま登録されるバグを修正し、初期値として計画数 (`planned_quantity`) が自動設定されるように変更。
  - 既存商品の計画数が変更された際にも、特売引当数が計画数と同期して更新されるように修正。
...
- **旧価格在庫および仕掛管理（WIP）の表示単位の適正化**:
  - 在庫数調整（棚卸し）ダイアログおよび米袋在庫状況テーブルにおいて、旧価格在庫の表示単位が「枚」に固定されていた不具合を修正。
  - 仕掛管理（WIP）ダイアログにおいて、「枚」と「m」の単位判定ロジックが簡易的（shapeに「枚」が含まれているか）だったため、共通関数 `isRollBag` を使用した堅牢な判定にアップデート。これにより「単袋」などの他の袋形状で正しく「枚」が表示されるようにバグを修正。
  - 商品タイプ（ロール袋の場合は「m」、それ以外は「枚」）に応じて動的に正しい単位が表示されるように改善。
...
>>>>>>> 9f89e427ce9ee6c847b26a12919c1994b956f90f
